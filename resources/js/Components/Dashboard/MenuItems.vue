            ...
            <div class="basis-3/4 overflow-x-auto">
                <div class="overflow-hidden sm:rounded-md mb-4">
                    <div v-for="item in highLevelItems" :key="item.id" class="flex items-center">
                        ...
                        <div v-for="child in getChildren(item.id)"
                            class="p-3 mr-1 mb-2 border border-gray-200 mb-2 bg-white  w-[250px]">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="ml-2 flex-shrink-0 flex space-x-2">
                                    ...
                                    <button @click="deleteMenuItem(child)" class="text-red-600 hover:text-red-900"><svg
                        ...
                        <div @click="appendItem(item)"
                            class="px-3 text-gray-600 cursor-pointer hover:text-gray-300 transition-all">
                            <i class="fa fa-plus"></i>
                        </div>
                    </div>
                    <div @click="modal = true"
                        class="p-3 mr-1 mb-2 flex items-center cursor-pointer justify-center bg-gray-300 text-gray-600 hover:text-gray-400 hover:bg-gray-200 w-[250px] transition-all">
                        <i class="fa fa-plus"></i>
                    </div>
                </div>
                ...
                <Transition name="fade" mode="out-in">
                    <div v-if="modal"
                        ...
                        <form @submit.prevent="submitForm" class="container">
                            ...
                            <div class="mb-6">
                                <label class="p-2">Отображать в мобильном меню <input class="w-4 h-4 ml-4"
                                        type="checkbox" v-model="form.is_mobile"></label>
                            </div>
...
export default {
    ...
    methods: {
        ...
        async fetchMenuItems() {
            try {
                axios.get(`/dashboard/menu-items/${this.type}`).then(response => {
